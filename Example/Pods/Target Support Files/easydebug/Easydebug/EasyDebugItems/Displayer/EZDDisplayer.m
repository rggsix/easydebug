...
- (void)setupBaseUI{
    ...
    self.displayerSwitch.size = CGSizeMake(60, 60);
}
...
- (void)setupToWindow:(UIWindow *)window{
    ...
    self.displayerSwitch.ezd_origin = CGPointMake(window.ezd_width * .1, window.ezd_height - displayer.displayerSwitch.height * .4);
...
- (void)displayerSwitchClicked{
    ...
    if (!comeOut) {
        ...
        [UIView animateWithDuration:0.25 delay:0 usingSpringWithDamping:0.75 initialSpringVelocity:0.5 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.displayerSwitch.y -= self.displayerSwitch.height * .6;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"keyWindow"]) {
        [self setupToWindow:kHCKeyWindow];
